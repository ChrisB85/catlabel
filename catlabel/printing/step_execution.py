...
async def _execute_query_step(
    ...
    *,
    ...
    if step.repeat_interval_sec is None:
        return await _query_once(session, step, timeout=timeout, log_prefix=log_prefix)
...
async def _query_once(
    ...
    *,
    ...
    if session.can_query_control_packet():
        ...
        raise RuntimeError(
            f"Protocol query {step.label!r} requires request/reply transport support"
        )
    session.report_debug(
        f"{log_prefix} query {step.label}: tx={bytes_preview(step.data)} "
        f"rx={bytes_preview(reply)}"
    )
    return reply
